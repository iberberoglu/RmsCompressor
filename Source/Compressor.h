...
//    }
    ...
    float processSample(float input, float rmsLevel) {
        ...
        if (rmsLvl > thresholdLevel) {
            ...
            /* TEST AMAÇLI
            float inputDB = Decibels::gainToDecibels(input);
            float compressedSampleDB = Decibels::gainToDecibels(compressedSample);
            float rmsLvlDB = Decibels::gainToDecibels(rmsLvl);
            float thresholdDB = Decibels::gainToDecibels(thresholdLevel);
            */
